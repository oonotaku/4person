...
- 発案者のアウトプットをコンパクト化
  - 出力フォーマット：💡タイトル（15文字以内）＋・箇条書き3点（各20文字以内）
  - 前置き・まとめ文・マークダウン記法（**、-）を禁止
  - バックエンドで `・` 前に `\n`、`💡` 前に `\n\n` を挿入して改行を保証
- 調査者の出力改善
  - 1行目に判定バッジ（✅/⚠️/🚫）＋要約を出力するフォーマットに変更
  - 「SUMMARY:」プレフィックスを廃止・禁止
- 発案者吹き出しの改行処理
  - `whiteSpace: "pre-wrap"` インラインスタイルで改行を確実に表示
...
- 調査者の折りたたみUI（判定行のみデフォルト表示、詳細は「詳細を見る ▼」で展開）※実装中・未完了
- Phase 1の①ブラッシュアップの文言変更（「調査結果を踏まえて案を磨く」が伝わる表現に）
- Phase 1の選択ループ回数制限（①②は最大2回まで、3回目は③のみ選択可能に）
