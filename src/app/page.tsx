...
function HomeContent() {
  ...
  async function loadSession(id: string) {
    try {
      ...
      // セッションデータを読み込み（summary・decided_idea_title）
      try {
        const session = await getSession(id);
        if (session?.summary) {
          setSummary(session.summary as Summary);
        }
        if (session?.decided_idea_title) {
          setDecidedIdeaTitle(session.decided_idea_title);
        }
        ...
        console.error("[loadSession data error]", e);
      }
    } catch (e) {
                ...
                <div className="px-5 py-5 space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-800 flex items-center gap-1.5">
                      <span>✅</span>
                      <span>{extractResearcherVerdict(messages) || (lang === "ja" ? "—" : "—")}</span>
                    </p>
                  </div>
                </div>
