...
## 出力フォーマット（必須）
- 余計な前置き（「はい、以下に〜」「提案します」など）・まとめ文は不要。案をそのまま出力する
- 各案はタイトル行（「① 案名」形式）＋箇条書き3点以内で構成する
- 1案あたり最大5行を目安にする
...
1. サマリー部（3行以内）：
   - 調査対象の案と市場状況を1〜2文で要約
   - 最後の行に必ず「判定：[勝てる余地あり / 勝てる余地は限定的 / 参入障壁が高く厳しい]」を記載

2. マーカー行（必須）：
   <<<DETAIL>>>

3. 詳細レポート：
   - 競合調査・市場規模・法規制等の詳細を記述（文字数制限なし）
   - レポートの末尾に必ず以下の1行を追加する：
     「この結果を踏まえて、どうしますか？ ① この案をブラッシュアップする ② 全く新しい案を出してもらう ③ この案で次のフェーズに進む」

例：
市場は存在するが競合が多い。差別化次第で勝てる余地あり。
判定：勝てる余地あり
<<<DETAIL>>>
（競合A・B・Cが先行しているが、中小企業向けの低価格帯には空白がある。市場規模は〜億円。法規制は〜。）
この結果を踏まえて、どうしますか？ ① この案をブラッシュアップする ② 全く新しい案を出してもらう ③ この案で次のフェーズに進む
